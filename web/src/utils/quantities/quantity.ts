import type { BinaryScale, DecimalFormat, Format, Scale } from './types'
...
interface QuantityDisplayCommonOptions {
	/**
...
function divRoundAwayFromZero(n: bigint, d: bigint): { q: bigint, exact: boolean } {
	if (d <= 0n)
		throw new RangeError('divisor must be positive')
	if (n === 0n)
		return { q: 0n, exact: true }
	const a = absBigInt(n)
	const q = a / d
	const r = a % d
...
function normalizeDecimal(value: bigint, scale: number): { value: bigint, scale: number } {
	if (value === 0n)
...
function roundToMinScale(value: bigint, scale: number, minScale: number): { value: bigint, scale: number, exact: boolean } {
	if (value === 0n)
		return { value: 0n, scale: 0, exact: true }
	if (scale >= minScale)
		return { value, scale, exact: true }
...
function exceedsMaxAllowed(absUnscaled: bigint, scale: number): boolean {
	if (absUnscaled === 0n)
		return false
	if (scale >= 0) {
...
function capToMaxAllowed(value: bigint, scale: number): { value: bigint, scale: number, capped: boolean } {
	if (value === 0n)
		return { value: 0n, scale: 0, capped: false }
	const abs = absBigInt(value)
	if (!exceedsMaxAllowed(abs, scale))
		return { value, scale, capped: false }
	return { value: value < 0n ? -MAX_INT64 : MAX_INT64, scale: 0, capped: true }
}

function normalizeConstructedQuantity(value: bigint, scale: number): { value: bigint, scale: number } {
...
function normalizeConstructedBinaryScaledQuantity(value: bigint, scale: BinaryScale): { value: bigint, scale: number } {
	const capped = capToMaxAllowed(value * pow2BigInt(Number(scale)), 0)
	return normalizeDecimal(capped.value, capped.scale)
}

function compareAmounts(aValue: bigint, aScale: number, bValue: bigint, bScale: number): -1 | 0 | 1 {
	if (aScale === bScale) {
		if (aValue === bValue)
			return 0
...
function isNonZeroAbsLessThanOne(value: bigint, scale: number): boolean {
	if (value === 0n)
		return false
	if (scale >= 0)
		return false
...
interface InterpretResult { base: 10 | 2, exponent: number, format: Format }

function interpretSuffix(suffix: string): InterpretResult {
...
function constructSuffix(base: 10 | 2, exponent: number, format: Format): string {
	if (format === 'DecimalSI') {
		if (base === 10) {
			switch (exponent) {
				case -9:
...
interface CanonicalNumber { mantissa: string, exponent: number }
...
function scaledInt(value: bigint, valueScale: number, targetScale: number): { q: bigint, exact: boolean } {
	if (value === 0n)
		return { q: 0n, exact: true }
	const diff = valueScale - targetScale
	if (diff === 0)
		return { q: value, exact: true }
	if (diff > 0)
		return { q: value * pow10BigInt(diff), exact: true }
	const d = pow10BigInt(-diff)
...
function parseQuantityString(input: string): ParsedParts {
...
function parseFromParts(parts: ParsedParts): { value: bigint, scale: number, format: Format } {
...
function isBinaryScaleValue(scale: unknown): scale is BinaryScale {
...
function resolveDecimalDisplayFormat(fallbackFormat: Format): DecimalFormat {
	return fallbackFormat === 'DecimalExponent' ? 'DecimalExponent' : 'DecimalSI'
}

function clampToDecimalSIExponent(exp: number): number {
...
function clampToBinarySIBits(bits: number): number {
...
function chooseAutoDecimalExponent(absValue: bigint, scale: number): number {
	...
	for (let i = DECIMAL_SI_EXPONENTS.length - 1; i >= 0; i--) {
		...
		if (compareAmounts(absValue, scale, 1n, exp) >= 0)
			return exp
	}
	return -9
}

function chooseAutoBinaryBits(absValue: bigint, scale: number): number {
	...
	for (let i = BINARY_SI_BITS.length - 1; i >= 0; i--) {
		...
		if (compareAmounts(absValue, scale, pow2BigInt(bits), 0) >= 0)
			return bits
	}
	return 0
}

function buildMantissaRationalForDecimal(value: bigint, valueScale: number, targetExp: number): { numer: bigint, denom: bigint } {
	// mantissa = (value * 10^valueScale) / 10^targetExp
	if (value === 0n)
		return { numer: 0n, denom: 1n }
...
function buildMantissaRationalForBinary(value: bigint, valueScale: number, targetBits: number): { numer: bigint, denom: bigint } {
...
function binaryScaledInt(value: bigint, valueScale: number, targetBits: number): { q: bigint, exact: boolean } {
	const r = buildMantissaRationalForBinary(value, valueScale, targetBits)
	return divRoundAwayFromZero(r.numer, r.denom)
}

function roundedAbsScaled(numer: bigint, denom: bigint, digits: number): bigint {
	...
	if (r * 2n >= denom)
		q += 1n // round half-up (ties away from 0 in magnitude)
	return q
}

function formatRationalDecimal(numer: bigint, denom: bigint, digits: number): string {
	if (denom <= 0n)
...
export class Quantity {
	...
	roundUp(scale: Scale): { result: Quantity, exact: boolean } {
		...
		if (this.#value === 0n)
			return { result: this, exact: true }
		if (this.#scale >= target)
			return { result: this, exact: true }
